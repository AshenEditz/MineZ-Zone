...
const GITHUB_TOKEN = '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';
...
// Initialize Firebase (Database and Auth only)
...
async function uploadToGitHub(file, type, category) {
...
// Handle Upload with GitHub Storage
async function handleUpload(e) {
    ...
    showLoading();
    console.log('📤 Starting upload to GitHub:', name);
    
    try {
        // Upload image to GitHub
        console.log('📸 Uploading image to GitHub...');
        showNotification('Uploading image...', 'info');
        const imageResult = await uploadToGitHub(imageFile, type, 'images');
        ...
        // Upload content file to GitHub (if not skin)
        if (type !== 'skins' && contentFile) {
            console.log('📁 Uploading file to GitHub...');
            showNotification('Uploading file...', 'info');
            const fileResult = await uploadToGitHub(contentFile, type, 'files');
...
// Handle Server Upload with GitHub Storage
async function handleServerUpload(e) {
    ...
    showLoading();
    console.log('🖥️ Adding server with GitHub storage:', name);
    
    try {
        // Upload image to GitHub
        showNotification('Uploading server image...', 'info');
        const imageResult = await uploadToGitHub(imageFile, 'servers', 'images');
        const imageUrl = imageResult.url;
...
console.log('✅ MineZ Zone loaded - GitHub Storage Mode');