...
function updateUIForUser() {
    ...
    if (currentUser.username === 'AshenGamerZ') {
        addAdminLink();
    }
...
function loadSectionData(section) {
    switch(section) {
        ...
        case 'profile':
            ...
            break;
        case 'admin':
            if (currentUser && currentUser.username === 'AshenGamerZ') {
                loadAdminPanel();
            }
            break;
    }
...
function initializeUploadButtons() {
    ...
    if (uploadAddonBtn) {
        uploadAddonBtn.addEventListener('click', () => openUploadModal('addons'));
    }
    if (uploadTemplateBtn) {
        uploadTemplateBtn.addEventListener('click', () => openUploadModal('templates'));
    }
    if (uploadShaderBtn) {
        uploadShaderBtn.addEventListener('click', () => openUploadModal('shaders'));
    }
    if (uploadSkinBtn) {
        uploadSkinBtn.addEventListener('click', () => openUploadModal('skins'));
    }
    if (uploadServerBtn) {
        uploadServerBtn.addEventListener('click', () => openServerModal());
    }
    ...
    if (serverForm) {
        serverForm.addEventListener('submit', handleServerUpload);
    }
}
...
async function handleUpload(e) {
    ...
    try {
        ...
        const imageRef = storage.ref(`images/${type}/${Date.now()}_${imageFile.name}`);
        ...
        // Upload file for non-skins
        if (type !== 'skins' && contentFile) {
            const fileRef = storage.ref(`files/${type}/${Date.now()}_${contentFile.name}`);
            const fileSnapshot = await fileRef.put(contentFile);
            fileUrl = await fileSnapshot.ref.getDownloadURL();
...
async function handleServerUpload(e) {
    ...
    try {
        const imageRef = storage.ref(`images/servers/${Date.now()}_${imageFile.name}`);
        const imageSnapshot = await imageRef.put(imageFile);
        const imageUrl = await imageSnapshot.ref.getDownloadURL();
...
function initializeChatWidget() {
    ...
    if (chatAdminBtn) {
        chatAdminBtn.addEventListener('click', openChat);
    }
